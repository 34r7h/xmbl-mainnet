...
2. **`simulator`** — DONE: revived as `LocalDevnet` with a gated suite (`src/devnet.test.mjs`, in `test:protocol`). Remaining: wire the RPC/extension bridge (§ `DEVNET-SEAM-FINDING.md`), and fix the two PINNED consensus→ledger verification seams (an audit-level signature-domain decision).
