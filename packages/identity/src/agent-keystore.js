...
// keys/ first, legacy root as fallback — the master key derives every node identity, so losing it is
// unrecoverable and a migration must never make it unfindable. Mirrors xmbl-node-setup.mjs: a reader
// that only knows the legacy root mints a FRESH key there when the real one has moved to keys/,
// silently forking the box onto two master keys.
function defaultMasterKeyPath() {
  const h = path.join(os.homedir(), '.handoff');
  const moved = path.join(h, 'keys', 'xmbl-master.key');
  try { if (fs.existsSync(moved)) return moved; } catch { /* fall through */ }
  return path.join(h, 'xmbl-master.key');
}
