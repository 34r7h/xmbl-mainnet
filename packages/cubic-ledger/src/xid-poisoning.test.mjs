...
const genuine = (label) => micromineTx({ type: 'anchor', event: 'task.created', hash: sha(label), ts: 1789500000000 });
...
// ── 1. THE ATTACK, IN ITS WORST ORDER: the forgery arrives FIRST ──
{
  ...
  ok('the genuine xid was NOT evicted', !l._evicted.has(`xid:${real.xid}`));
  ...
  ok('THE CORRECT ANCHOR IS STILL ADMITTED after its xid was impersonated', !!res && res.evicted !== true);
  ok('the genuine anchor is in the ledger (1 block)', l._membershipPool.length === 1);

  let again = null;
  try { await l.addTransaction(forgery); } catch (e) { again = e; }
  ok('a resubmission of the SAME forgery is still refused', again !== null);
  ok('and the genuine anchor survived that too', l._membershipPool.length === 1);
  await l.close?.(); rmSync(dir, { recursive: true, force: true });
}

// ── 2. THE OTHER ORDER: the genuine anchor is already stored when the forgery arrives ──
{
  const dir = mkdtempSync(join(tmpdir(), 'xmbl-poison-b-'));
  const l = new Ledger({ dbPath: dir }); await l.initialize?.();
  const real = genuine('victim-b');
  await l.addTransaction(real);
  const before = l._membershipPool.length;
  ok('the genuine anchor is stored first (1 block)', before === 1);

  const forgery = { ...real, hash: sha('forged-body-2') };
  let threw = null;
  try { await l.addTransaction(forgery); } catch (e) { threw = e; }
  ok('the forgery is refused', threw !== null && threw.code === 'XID_MISMATCH');
  ok('THE STORED BLOCK WAS NOT DELETED — evict() never ran against the genuine xid', l._membershipPool.length === before);
  ok('the genuine xid is still absent from the eviction set', !l._evicted.has(`xid:${real.xid}`));
  await l.close?.(); rmSync(dir, { recursive: true, force: true });
}
...
// ── 3. THE FIX DID NOT WEAKEN EVICTION: a genuinely invalid datum is still refused for good ──
{
  ...
  ok('and the genuine anchor still gets in', !!r && r.evicted !== true);
  await l.close?.(); rmSync(dir, { recursive: true, force: true });
}
...
// silently dropped. Same denial-of-service, different door.
{
  ...
  ok('a forgery sharing the genuine event:hash is refused', threw !== null && threw.code === 'XID_MISMATCH');
  ...
  ok('THE CORRECT ANCHOR IS STILL ADMITTED, not answered "duplicate"', res && res.duplicate !== true);
  ok('and it is really stored (1 block)', l._membershipPool.length === 1);
  const dupe = await l.addTransaction(real);
  ok('a genuine duplicate of the genuine anchor is still deduped', dupe && dupe.duplicate === true);
...
// node traffic. A fix that handles one entry point and not the other is not a fix.
{
  ...
  ok('SEAL PATH: THE CORRECT ANCHOR IS STILL ADMITTED after the forgery', !!res);
...
// ── 6. ONE BAD TRANSACTION MUST NOT DISCARD THE GENUINE ONES BATCHED WITH IT ──
// addSealedBatch takes a LIST. A throw out of the middle of the loop drops every remaining entry on the floor,
// so one forgery placed ahead of genuine traffic silently deletes it — a cheaper denial of service than either
// door above, needing no xid collision at all.
{
  const dir = mkdtempSync(join(tmpdir(), 'xmbl-poison-f-'));
  const l = new Ledger({ dbPath: dir }); await l.initialize?.();
  const a = genuine('batch-a'), b = genuine('batch-b'), c = genuine('batch-c');
  const poison = { ...genuine('batch-poison'), nonce: 7 };         // invalid: body no longer mines to its xid
  let threw = null;
  ...
  ok('THE THREE GENUINE TRANSACTIONS BEHIND IT ARE ALL ADMITTED', l._membershipPool.length === 3);
