...
export const NODE_CONFIG_SCHEMA = {
  ...
  // libp2p multiaddrs this node ADVERTISES to peers, when they differ from what it listens on — the
  // NAT case, which is every cloud box and every laptop. Empty (the default) keeps the old behaviour
  // exactly: libp2p announces whatever interfaces it discovered, which for a NAT'd host means private
  // addresses no peer can dial. Set this to the address the operator knows is reachable.
  announce_addrs: { type: 'string[]', default: [] },
