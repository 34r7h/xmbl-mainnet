  ...
  setup() {
    ...
    function connectSocket() {
      ...
      socket.on('xclt:block:added', (data) => {
        ...
        addActivity('@xmbl/cubic-ledger', `Block ${blockId} added`);
      });
      ...
      socket.on('xclt:face:complete', async (data) => {
        ...
        addActivity('@xmbl/cubic-ledger', `Face ${faceIdx} completed`);
      ...
      socket.on('xclt:cube:complete', (data) => {
        ...
        addActivity('@xmbl/cubic-ledger', `Cube ${cubeId} completed (Level ${level})`);
        // Update cube outline visualization
        updateCubeVisualization(data);
      });

      socket.on('xclt:supercube:complete', (data) => {
        addActivity('@xmbl/cubic-ledger', `Super-cube ${data.superCubeId} completed (Level ${data.level})`);
      });
      ...
      socket.on('xid:identity:created', (data) => {
        ...
        addActivity('@xmbl/identity', `Identity ${address.substring(0, 8)} created`);
      });
      ...
      socket.on('xpc:transaction:new', (data) => {
        ...
        addActivity('@xmbl/consensus', `TX ${txId}: ${txType}`);
      });
      ...
      socket.on('xvsm:state:diff', (data) => {
        ...
        addActivity('@xmbl/state-machine', `State diff: ${changeCount} changes`);
      });
      ...
      socket.on('xsc:operation', (data) => {
        ...
        addActivity('@xmbl/storage-compute', `${data.type} operation`);
      });
      ...
      socket.on('xsc:compute:operation', (data) => {
        ...
        addActivity('@xmbl/storage-compute', `Compute: ${data.functionName}`);
