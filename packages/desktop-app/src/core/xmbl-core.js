...
class XMBLCore {
  ...
  async _loadModules() {
    ...
    const { XNNode } = await import('@xmbl/networking');
    const { Ledger } = await import('@xmbl/cubic-ledger');
    const { StateMachine } = await import('@xmbl/state-machine');
    const { ConsensusWorkflow } = await import('@xmbl/consensus');
    const { StorageNode, MarketPricing } = await import('@xmbl/storage-compute');
  ...
  async init() {
    ...
    const { ConsensusGossip } = await import('@xmbl/consensus');
